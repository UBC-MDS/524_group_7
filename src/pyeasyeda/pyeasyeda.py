...
def birds_eye_view(df, n=20, var_list=None):
    ...
    if var_list is None:
        ...
        for num_col in numeric:
            chart = sns.histplot(df, x=(num_col), bins=n, kde=True).set(
                title="Histogram for " + num_col
            )
            plt.figure()
            charts.append(chart)
        ...
        for cat_col in categorical:
            if len(df[cat_col].value_counts()) > 10:
                ...
                chart = sns.countplot(data=df, x=(cat_col)).set(
                    title="Bar Chart for " + col
                )
                plt.figure()
                charts.append(chart)
        ...
        chart = sns.heatmap(
            corr_matrix, vmin=-1, vmax=1, annot=True, cmap="BrBG", mask=mask
        ).set_title(
    ...
    # Plot just the custom variables from var_list (if applicable)
        ...
        for custom_col in var_list:
            ...
            if custom_col in numeric:
                ...
                chart = sns.histplot(df, x=(custom_col), bins=n, kde=True).set(
                    title="Histogram for " + custom_col
                )
                plt.figure()
                charts.append(chart)
            ...
            # Bar Charts
                ...
                if len(df[custom_col].value_counts()) > 10:
                    ...
                    chart = sns.countplot(data=df, x=(custom_col)).set(
                        title="Bar Chart for " + custom_col
                    )
                    plt.figure()
                    charts.append(chart)
            else:
